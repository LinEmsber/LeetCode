...
int maxIncreaseKeepingSkyline(int** grid, int gridRowSize, int *gridColSizes)
{
	...
	/* Find the highest buildings in the line of each rows and columns. */
	for( i = 0; i < gridRowSize; i++ )
		for( j = 0; j < *gridColSizes; j++ ){
			array_row[i] = MAX(array_row[i], grid[i][j]);
			array_col[j] = MAX(array_col[j], grid[i][j]);
		}
	...
	for(i = 0; i< gridRowSize; i++ )
		for( j = 0; j < *gridColSizes; j++ )
			ans += MIN(array_row[i], array_col[j]) - grid[i][j];
